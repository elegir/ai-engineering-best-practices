...
2. **Apply** — evaluate another repository against these practices and propose (not silently execute) a plan. Start with `playbooks/which-practices-apply.md` (infer the repo's facts with evidence, confirm them with Martin in one screen, list what applies and what is skipped with a reason about *that* repo), then `playbooks/audit-repo-against-kb.md` on the practices that apply. Each practice declares `applies-when` over the vocabulary in `practices/facts.md`; the same line is the "Applies when" column of `practices/README.md`. Never ask Martin a question whose answer is visible in the repo.
...
- `practices/` — one folder per practice: the *how*, with copyable files. Two kinds: `working-style` (any repo an agent works in) and `capability` (depends on what the product does; attaches on facts such as `retrieval`, `tools`, `acts_on_world`, `multi_tenant`). `practices/facts.md` is the vocabulary (docs skeletons, `AGENTS.md` model, hook scripts, `PROGRESS.json`, worktree scripts, smoke tests per app type, spec commands, prompts, token checklist) and stack variants. Start at `practices/README.md`. Copy and adapt; never copy blindly.
...
| find out which practices apply to a repo before auditing it | `decisions/0003-applicability-by-facts.md` | `playbooks/which-practices-apply.md`, `practices/facts.md`, `practices/README.md` |
