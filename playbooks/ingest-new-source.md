...
3b. **Update practices.** Any *new* practice folder must declare `kind` (`working-style` | `capability`) and `applies-when` (one line using only words from `practices/facts.md`) in its README frontmatter **and** get its row in the `practices/README.md` table; if a fact word is missing from the vocabulary, add it to `practices/facts.md` with evidence and a change-log line (decision 0003).
